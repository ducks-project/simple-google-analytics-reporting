...
class Service
{
    ...
    public function __construct(
        ...
        $this->auth = new AssertionCredentials(
    ...
    {
        $token = $this->auth->getLastReceivedToken();
    ...
    public function __call($name, $parameters)
    {
        ...
        $parameter_key = static::ArrayKeyExists($name, $this->report_root_parameters);

        if ($parameter_key) {
            return $this->report_root_parameters[$parameter_key];
        }

        $aggregate_metric_key = static::ArrayKeyExists($name, $this->report_aggregate_metrics);
    ...
    {
        $camel = u($name)->camel();
        if (\method_exists(static::class, $camel)) {
            return \call_user_func_array([static::class, 'arrayKeyExists'], $arguments);
        }
    }
